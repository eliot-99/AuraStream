...
export default function BoxesBackground() {
    ...
    return (_jsxs("div", { ref: ref, className: "absolute inset-0 -z-20 overflow-hidden bg-slate-900", children: [_jsx("div", { className: "absolute inset-0 pointer-events-none z-20 bg-slate-900 [mask-image:radial-gradient(transparent,white)]" }), _jsx("div", { className: "absolute inset-0 grid", style: {
                    gridTemplateColumns: `repeat(${grid.cols}, minmax(0, 1fr))`,
                    gridTemplateRows: `repeat(${grid.rows}, minmax(0, 1fr))`,
                    transform: 'translate(var(--tx, 0px), var(--ty, 0px))',
                    transition: 'transform 120ms ease-out',
                }, children: cells.map((i) => (_jsx("div", { className: "relative", style: {
                        // Box style
                        outline: '1px solid rgba(255,255,255,0.06)',
                        background: 'radial-gradient(120px 80px at 50% 50%, rgba(255,255,255,0.04), transparent 60%)',
