...
void main() {
    ...
    gl_FragColor = vec4(color * t * finalFade * opacity, alpha);
        ...
        const program = new Program(renderer.gl, { vertex: vert, fragment: frag, uniforms });
        ...
        const handleMouseEnter = () => { if (!mouseInteraction)
            return; mouseInfluenceRef.current = 1.0; };
        const handleMouseLeave = () => { if (!mouseInteraction)
            return; mouseInfluenceRef.current = 0.0; };
    ...
    return (_jsx("div", { ref: containerRef, className: "w-full h-full relative overflow-hidden [&_canvas]:block" }));
};
