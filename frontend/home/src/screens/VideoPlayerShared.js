    ...
    useEffect(() => {
        ...
        const checkPeerStream = () => {
            try {
                ...
                const remotePanelEl = document.querySelector('video[data-peer="remote"]');
                const remoteTopEl = document.querySelector('video[data-remote-stream]');
                let sourceStream = null;
                if (remotePanelEl && remotePanelEl.srcObject) {
                    sourceStream = remotePanelEl.srcObject;
                }
                else if (remoteTopEl && remoteTopEl.srcObject) {
                    sourceStream = remoteTopEl.srcObject;
                }
                // Also check global window objects from SharedRoom
                if (!sourceStream && window.__peerStream) {
                    sourceStream = window.__peerStream;
                }
                if (sourceStream && peerVideoRef.current) {
                    peerVideoRef.current.srcObject = sourceStream;
