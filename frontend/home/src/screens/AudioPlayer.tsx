    ...
    <div className="relative min-h-screen w-full font-montserrat text-white">
      ...
      <div className={`fixed top-0 right-0 h-full w-[80%] max-w-[380px] z-30 bg-black/70 backdrop-blur-md border-l border-white/15 transform transition-transform duration-300 ${showDrawer ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
          <div className="text-white/90 font-medium">Playlist</div>
          <div className="flex items-center gap-2">
            <button onClick={handleDirectory} aria-label="Select Directory" title="Select Directory" className="h-9 w-9 rounded bg-white/10 border border-white/20 text-white/90 flex items-center justify-center hover:scale-105 transition">
                  ...
                  <button
                    onClick={async () => {
                      ...
                      if (a) {
                        ...
                        a.src = t.url;
                        a.load();
                        try { await ensureAudioReady(); await a.play(); setPlaying(true); } catch { setPlaying(false); }
                      }
                    }}
                    className={`w-full flex items-center gap-3 px-3 py-2 rounded border ${i === index ? 'bg-white/15 border-white/30' : 'bg-white/5 border-white/10 hover:bg-white/10'} text-left`}
                  >
                    <img src={t.cover || FALLBACK_COVER} className="w-10 h-10 rounded object-cover" alt="" />
